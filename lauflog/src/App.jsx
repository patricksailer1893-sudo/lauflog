...
function KpiCard({ label, value, sub, accent = "#FFFFFF", border }) {
  return (
    <div style={{ background: "#111111", border: `1px solid ${border || "#222222"}`, borderRadius: 16, padding: "22px 24px" }}>
      <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 10, fontWeight: 600 }}>{label}</div>
      <div style={{ fontSize: 30, fontWeight: 900, color: accent, fontFamily: "Inter, sans-serif", lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ fontSize: 12, color: "#666666", marginTop: 8 }}>{sub}</div>}
    </div>
  );
}
  ...
  return (
    <div>
      <div
        ...
        style={{ border: `2px dashed ${drag ? "#E31837" : "#222222"}`, borderRadius: 12, padding: "22px 20px", textAlign: "center", cursor: "pointer", background: drag ? "#E3183708" : "transparent" }}
      >
        <div style={{ fontSize: 28, marginBottom: 6 }}>📂</div>
        <div style={{ fontSize: 13, color: "#AAAAAA" }}>GPX oder TCX Datei ablegen oder klicken</div>
        <div style={{ fontSize: 11, color: "#444444", marginTop: 4 }}>Strava: Aktivität → „…" → „Exportieren als GPX" oder „Exportieren als TCX"</div>
        <input ref={ref} type="file" accept=".gpx,.tcx" style={{ display: "none" }} onChange={e => process(e.target.files[0])} />
      </div>
      {status && (
        <div style={{ marginTop: 8, padding: "8px 12px", borderRadius: 8, fontSize: 12, background: status === "ok" ? "#E3183711" : "#f43f5e11", border: `1px solid ${status === "ok" ? "#E3183744" : "#f43f5e44"}`, color: status === "ok" ? "#E31837" : "#f43f5e" }}>
          {msg}
        </div>
      )}
    </div>
...
function Inp({ k, ph, type = "text", form, set }) {
  return (
    <input type={type} placeholder={ph} value={form[k] ?? ""} onChange={e => set(k, e.target.value)}
      style={{ background: "#000000", border: "1px solid #1e2436", borderRadius: 7, padding: "9px 12px", color: "#FFFFFF", fontSize: 13, width: "100%", outline: "none", fontFamily: "inherit" }} />
  );
}
...
function WorkoutForm({ onSave, onCancel, initial }) {
  ...
  return (
    <div style={{ background: "#111111", border: "1px solid #1e2436", borderRadius: 14, padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
      {!initial && (
        <div style={{ display: "flex", gap: 4, background: "#000000", borderRadius: 8, padding: 4 }}>
          {[["gpx","📂 Strava GPX"],["manual","✏️ Manuell"]].map(([k, l]) => (
            <button key={k} onClick={() => setTab(k)}
              style={{ flex: 1, background: tab === k ? "#222222" : "transparent", border: "none", borderRadius: 6, color: tab === k ? "#FFFFFF" : "#5a6480", padding: "8px 0", cursor: "pointer", fontSize: 12, fontWeight: tab === k ? 700 : 400 }}>
              {l}
            </button>
          ))}
        </div>
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <select value={form.type} onChange={e => set("type", e.target.value)}
              style={{ background: "#000000", border: "1px solid #1e2436", borderRadius: 7, padding: "9px 12px", color: "#FFFFFF", fontSize: 13, outline: "none" }}>
              {TYPE_GROUPS.map(g => (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            <div>
              <div style={{ fontSize: 10, color: "#E31837", marginBottom: 4, letterSpacing: 1 }}>↑ AUFSTIEG (m)</div>
          ...
          <div>
            <div style={{ fontSize: 10, color: "#666666", letterSpacing: 1.5, marginBottom: 8, textTransform: "uppercase" }}>HF-Zonen %</div>
          ...
          <div>
            <div style={{ fontSize: 10, color: "#666666", letterSpacing: 1.5, marginBottom: 8, textTransform: "uppercase" }}>Tempo-Zonen % <span style={{ color: "#444444", fontWeight: 400, fontSize: 9 }}>(Strava SAT)</span></div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 6 }}>
              {PACEZ.map(z => (
                <div key={z.key}>
                  <div style={{ fontSize: 9, color: z.color, marginBottom: 2 }}>{z.label} {z.name}</div>
                  <div style={{ fontSize: 8, color: "#444444", marginBottom: 3 }}>{z.range}</div>
                  <input type="number" placeholder="%" min={0} max={100} value={form[z.key] ?? ""} onChange={e => set(z.key, e.target.value)}
                    style={{ background: "#000000", border: `1px solid ${z.color}44`, borderRadius: 7, padding: "7px 6px", color: "#FFFFFF", fontSize: 12, width: "100%", outline: "none" }} />
                </div>
              ))}
            </div>
          ...
          <div>
            <div style={{ fontSize: 10, color: "#666666", letterSpacing: 1.5, marginBottom: 8, textTransform: "uppercase" }}>Wie war das Training?</div>
            <div style={{ display: "flex", gap: 8 }}>
              {MOODS.map((emoji, i) => (
                <button key={emoji} onClick={() => set("mood", emoji)}
                  style={{ flex: 1, padding: "10px 0", borderRadius: 8, border: form.mood === emoji ? `2px solid #4ade80` : "1px solid #1e2436", background: form.mood === emoji ? "#E3183715" : "#000000", cursor: "pointer", fontSize: 22, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
                  <span>{emoji}</span>
                  <span style={{ fontSize: 9, color: form.mood === emoji ? "#E31837" : "#666666" }}>{MOOD_LABELS[i]}</span>
                </button>
              ))}
            </div>
          </div>
          <textarea placeholder="Notizen…" value={form.notes} onChange={e => set("notes", e.target.value)} rows={2}
            style={{ background: "#000000", border: "1px solid #1e2436", borderRadius: 7, padding: "9px 12px", color: "#FFFFFF", fontSize: 13, outline: "none", resize: "vertical", fontFamily: "inherit" }} />
        </>
      )}
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
        ...
        {tab === "manual" && (
          <button onClick={() => { if (!form.date || !form.distance) return; onSave({ ...form, id: initial?.id || Date.now() }); }}
            style={{ background: "#E31837", border: "none", borderRadius: 7, color: "#050810", padding: "9px 22px", cursor: "pointer", fontSize: 13, fontWeight: 800 }}>
            Speichern
          </button>
        )}
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontSize: 11, color: "#666666", letterSpacing: 2, textTransform: "uppercase" }}>{workouts.length} Einheiten</div>
        <button onClick={onAdd} style={{ background: "#E31837", border: "none", borderRadius: 8, color: "#050810", padding: "9px 18px", cursor: "pointer", fontSize: 13, fontWeight: 800 }}>+ Einheit</button>
      </div>
      {workouts.length === 0 && (
        <div style={{ textAlign: "center", padding: 60, color: "#333333", fontSize: 14 }}>Noch keine Einheiten — GPX importieren oder manuell eintragen!</div>
      )}
      {sorted.map(w => {
        ...
        return (
          <div key={w.id} style={{ background: "#111111", border: `1px solid ${isCfm ? "#f43f5e44" : "#222222"}`, borderRadius: 12, padding: "16px 20px", display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {w.notes && <div style={{ fontSize: 12, color: "#666666", fontStyle: "italic" }}>{w.notes}</div>}
          </div>
        );
      })}
    </div>
  ...
  if (!workouts.length) {
    return (
      <div style={{ textAlign: "center", padding: "80px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
        ...
        <div style={{ color: "#666666", fontSize: 15 }}>Noch keine Einheiten vorhanden.</div>
        <button onClick={onAdd} style={{ background: "#E31837", border: "none", borderRadius: 10, color: "#050810", padding: "12px 28px", cursor: "pointer", fontSize: 14, fontWeight: 800 }}>Erste Einheit hinzufügen</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        {/* Ruhetag */}
        {restRec && (
          <div style={{ background: "#111111", border: `1px solid ${restRec.color}33`, borderRadius: 14, padding: 18, display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase" }}>Heute</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: restRec.color }}>{restRec.msg}</div>
            <div style={{ fontSize: 12, color: "#666666" }}>{restRec.sub}</div>
          </div>
        )}
        {trainedToday && (
          <div style={{ background: "#111111", border: "1px solid #4ade8033", borderRadius: 14, padding: 18 }}>
            <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>Heute</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#E31837" }}>Training erledigt ✓</div>
            <div style={{ fontSize: 12, color: "#666666", marginTop: 4 }}>{workouts.filter(w=>w.date===t).length} Einheit{workouts.filter(w=>w.date===t).length>1?"en":""} heute</div>
          </div>
        )}

        {/* Körpermetriken */}
        <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 18 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase" }}>Körper heute</div>
            <button onClick={() => setShowMetricForm(v => !v)}
              style={{ background: showMetricForm ? "#222222" : "transparent", border: "1px solid #1a1f35", borderRadius: 6, color: "#666666", padding: "4px 10px", cursor: "pointer", fontSize: 11 }}>
          ...
          {showMetricForm && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
              ...
              <button onClick={saveMetric}
                style={{ background: "#E31837", border: "none", borderRadius: 7, color: "#050810", padding: "8px", cursor: "pointer", fontSize: 12, fontWeight: 800 }}>
                Speichern
              </button>
            </div>
          )}
          ...
          ) : <div style={{ fontSize: 12, color: "#333333" }}>Noch keine Körperdaten eingetragen.</div>}
        </div>
      </div>
      ...
      <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 18 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase" }}>Aktivitäts-Heatmap — letzte 26 Wochen</div>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: 10, color: "#666666" }}>weniger</span>
            {["#0f0f0f","#2a1a1a","#2d6a2d","#E31837"].map((c,i) => <div key={i} style={{ width: 10, height: 10, borderRadius: 2, background: c }} />)}
            <span style={{ fontSize: 10, color: "#666666" }}>mehr</span>
          </div>
        </div>
        <div style={{ display: "flex", gap: 4, overflowX: "auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 3, paddingTop: 2, flexShrink: 0 }}>
            {["Mo","Di","Mi","Do","Fr","Sa","So"].map(d => (
              <div key={d} style={{ fontSize: 9, color: "#666666", height: 12, lineHeight: "12px", width: 14, textAlign: "right", paddingRight: 2 }}>{d}</div>
            ))}
          </div>
          {/* Weeks grid */}
            ...
            {heatWeeks.map((week, wi) => (
              <div key={wi} style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                {week.map((d, di) => {
                  ...
                  const color = !d.has ? "#0f0f0f" : d.km > 15 ? "#E31837" : d.km > 8 ? "#2d6a2d" : d.km > 0 ? "#2a1a1a" : "#E3183766";
                  return (
      ...
      {weekZ2Data.length > 0 && (
        <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 18 }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>80/20 — Zone 2 pro Woche</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[...weekZ2Data].reverse().map((d, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ fontSize: 11, color: "#666666", minWidth: 110, flexShrink: 0 }}>{d.label}</div>
          ...
          <div style={{ marginTop: 12, fontSize: 11, color: "#666666" }}>Ziel: ≥ 70% Zone 2 pro Woche für optimalen Konditionsaufbau</div>
        </div>
      )}

      {/* ── Wochenvolumen-Trend + HF-Zonen ── */}
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 12 }}>
        <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: "18px 18px 10px" }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Wochenvolumen-Trend</div>
          <ResponsiveContainer width="100%" height={140}>
            <BarChart data={volTrend} barCategoryGap="30%">
              <CartesianGrid stroke="#1a1a1a" strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="label" tick={{ fill: "#666666", fontSize: 8 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: "#666666", fontSize: 9 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 8, fontSize: 12 }} labelStyle={{ color: "#FFFFFF" }} formatter={v => [`${v} km`]} />
              <Bar dataKey="km" radius={[4,4,0,0]}>
                {volTrend.map((e, i) => <Cell key={i} fill={e.cur ? "#E31837" : "#2a1a1a"} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 18 }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Ø HF-Zonen</div>
          {zAvg.length > 0 ? (
            <>
              ...
              <div style={{ marginTop: 10, padding: "7px 10px", borderRadius: 8, background: z2pct >= 70 ? "#E3183711" : "#f43f5e0d", border: `1px solid ${z2pct >= 70 ? "#E3183733" : "#f43f5e33"}` }}>
                <span style={{ fontSize: 11, color: z2pct >= 70 ? "#E31837" : "#f43f5e", fontWeight: 700 }}>80/20: Z2 {z2pct}% {z2pct >= 70 ? "✓" : "← Ziel ≥70%"}</span>
              </div>
            </>
          ) : <div style={{ color: "#333333", fontSize: 12 }}>Noch keine HF-Daten.</div>}
        </div>
      </div>

      {paceData.length >= 2 && (
        <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: "18px 18px 10px" }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Pace-Entwicklung</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.8fr", gap: 12 }}>
        ...
        <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 18 }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Letzte Einheiten</div>
          {recent.map(w => (
            <div key={w.id} onClick={() => onEdit(w)} style={{ borderBottom: "1px solid #111525", paddingBottom: 8, marginBottom: 8, cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: "#EEEEEE" }}>{w.type}</span>
                </div>
      ...
      <button onClick={onAdd} style={{ background: "#E31837", border: "none", borderRadius: 10, color: "#050810", padding: 13, cursor: "pointer", fontSize: 14, fontWeight: 800, width: "100%" }}>
        + Neue Einheit
      </button>
    </div>
  );
...
function AnalyseView({ workouts }) {
  if (!workouts.length) {
    return <div style={{ textAlign: "center", padding: 80, color: "#333333" }}>Keine Daten.</div>;
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {zAvg.length > 0 && (
        <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 20 }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Ø HF-Zonenverteilung</div>
          ...
          <div style={{ padding: "10px 14px", borderRadius: 8, background: z2pct >= 70 ? "#E3183711" : "#f43f5e0d", border: `1px solid ${z2pct >= 70 ? "#E3183733" : "#f43f5e33"}` }}>
            <span style={{ fontSize: 12, color: z2pct >= 70 ? "#E31837" : "#f43f5e", fontWeight: 700 }}>80/20: Zone 2 Ø {z2pct}% {z2pct >= 70 ? "✓ Gut!" : "— Ziel ≥ 70%"}</span>
          </div>
        </div>
      )}
      ...
      {hrData.length >= 2 && (
        <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: "18px 18px 10px" }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Herzfrequenz-Trend</div>
          <ResponsiveContainer width="100%" height={140}>
      ...
      <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: "18px 18px 10px" }}>
        <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>km pro Typ</div>
        <ResponsiveContainer width="100%" height={130}>
      ...
      {(() => {
        ...
        return (
          <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 20 }}>
            <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Ø Tempo-Zonenverteilung (Strava SAT)</div>
            <div style={{ display: "flex", height: 14, borderRadius: 6, overflow: "hidden", marginBottom: 12 }}>
      ...
      {(() => {
        ...
        return (
          <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 20 }}>
            <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>👤 Instructor Statistik</div>
                ...
                <div key={inst.name}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      {i > 2 && <span style={{ fontSize: 12, color: "#666666", width: 20, textAlign: "center" }}>{i+1}.</span>}
...
function TrendsView({ workouts }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* ── Meilensteine ── */}
      <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 18 }}>
        <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>🏅 Meilensteine</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
          {MILESTONES.map(m => {
            const done = totalMin >= m.min;
            return (
              <div key={m.min} style={{ background: "#000000", border: `1px solid ${done ? m.color+"44" : "#222222"}`, borderRadius: 10, padding: "10px 14px", opacity: done ? 1 : 0.35, textAlign: "center", minWidth: 80 }}>
                <div style={{ fontSize: 24 }}>{m.icon}</div>
                <div style={{ fontSize: 10, color: done ? m.color : "#666666", fontWeight: 700, marginTop: 4 }}>{m.label}</div>
              </div>
            );
          })}
        </div>
        {next && (
          <div style={{ background: "#000000", borderRadius: 10, padding: "12px 16px" }}>
            <div style={{ fontSize: 11, color: "#666666", marginBottom: 6 }}>Nächster Meilenstein: <strong style={{ color: next.color }}>{next.icon} {next.label}</strong></div>
            <div style={{ background: "#222222", borderRadius: 4, height: 8, overflow: "hidden" }}>
      ...
      {workouts.some(w => w.mood) && (
        <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 18 }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>😊 Stimmung nach Wochentag</div>
          <div style={{ display: "flex", gap: 8 }}>
            {moodDayData.map(d => {
              ...
              return (
                <div key={d.day} style={{ flex: 1, background: "#000000", borderRadius: 10, padding: "10px 6px", textAlign: "center", border: `1px solid ${d.avg ? color+"33" : "#222222"}` }}>
                  ...
                  {d.count > 0 && <div style={{ fontSize: 9, color: "#666666", marginTop: 2 }}>{d.count}×</div>}
                </div>
              );
            })}
          </div>
          <div style={{ marginTop: 12, fontSize: 11, color: "#666666" }}>
            {(() => {
      ...
      <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: 18 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase" }}>⚖️ Körperdaten</div>
          <button onClick={() => setShowForm(v => !v)}
            style={{ background: "#222222", border: "none", borderRadius: 7, color: "#FFFFFF", padding: "7px 14px", cursor: "pointer", fontSize: 12 }}>
            {showForm ? "✕" : "+ Eintragen"}
          </button>
        </div>
        {showForm && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 14, padding: 14, background: "#000000", borderRadius: 10 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
              <div>
                <div style={{ fontSize: 10, color: "#666666", marginBottom: 4 }}>DATUM</div>
        ...
        {metrics.length === 0 && <div style={{color:"#333333",fontSize:12}}>Noch keine Körperdaten. Trag heute deinen ersten Wert ein!</div>}
      </div>
    </div>
  );
}
...
function UebersichtView({ workouts }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", gap: 4, background: "#111111", border: "1px solid #1a1f35", borderRadius: 10, padding: 4, alignSelf: "flex-start" }}>
        {[["week","Woche"],["month","Monat"],["year","Jahr"]].map(([k, l]) => (
          <button key={k} onClick={() => setMode(k)}
            style={{ background: mode === k ? "#222222" : "transparent", border: "none", borderRadius: 7, padding: "8px 18px", color: mode === k ? "#FFFFFF" : "#666666", cursor: "pointer", fontSize: 12, fontWeight: mode === k ? 700 : 400 }}>
            {l}
          </button>
        ))}
      </div>
      ...
      {current && (
        <div style={{ background: "#111111", border: "1px solid #4ade8033", borderRadius: 14, padding: 18 }}>
          <div style={{ fontSize: 10, color: "#E31837", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>
      ...
      <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: "18px 18px 10px" }}>
        <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>
        ...
        <ResponsiveContainer width="100%" height={170}>
          <BarChart data={chartData} barCategoryGap="25%">
            ...
            <Tooltip contentStyle={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 8, fontSize: 12 }} labelStyle={{ color: "#FFFFFF" }} formatter={v => [`${v} km`]} />
            <Bar dataKey="km" radius={[4,4,0,0]}>
              {chartData.map((e, i) => <Cell key={i} fill={e.cur ? "#E31837" : "#2a1a1a"} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>

      <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, overflow: "hidden" }}>
        <div style={{ padding: "14px 20px 0", fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase" }}>
            ...
            <thead>
              <tr style={{ borderBottom: "1px solid #111525" }}>
                {["Zeitraum","km","Einh.","Zeit","Ø Pace","Ø HF"].map(h => (
                  <th key={h} style={{ textAlign: h === "Zeitraum" ? "left" : "right", padding: "10px 14px", color: "#666666", fontWeight: 600, fontSize: 10, letterSpacing: 1.2, textTransform: "uppercase" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {[...rows].reverse().map(r => {
                ...
                return (
                  <tr key={r.key} style={{ borderBottom: "1px solid #0a0d1a", background: isCur ? "#E3183708" : "transparent" }}>
      ...
      {cumData.length >= 2 && (
        <div style={{ background: "#111111", border: "1px solid #1a1f35", borderRadius: 14, padding: "18px 18px 10px" }}>
          <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Kumulative km</div>
          <ResponsiveContainer width="100%" height={130}>
...
function ZieleView({ workouts }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontSize: 10, color: "#666666", letterSpacing: 2, textTransform: "uppercase" }}>Meine Ziele</div>
        <button onClick={() => setShowForm(v => !v)}
          style={{ background: "#E31837", border: "none", borderRadius: 8, color: "#050810", padding: "9px 18px", cursor: "pointer", fontSize: 13, fontWeight: 800 }}>
          {showForm ? "✕ Abbrechen" : "+ Ziel hinzufügen"}
        </button>
      </div>

      {showForm && (
        <div style={{ background: "#111111", border: "1px solid #4ade8033", borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              <div style={{ fontSize: 10, color: "#666666", marginBottom: 4, textTransform: "uppercase" }}>Zieltyp</div>
              <select value={form.type} onChange={e => setForm(f => ({...f, type: e.target.value}))}
                style={{ background: "#000000", border: "1px solid #1e2436", borderRadius: 7, padding: "9px 12px", color: "#FFFFFF", fontSize: 13, outline: "none", width: "100%" }}>
          ...
          <div>
            <div style={{ fontSize: 10, color: "#666666", marginBottom: 4, textTransform: "uppercase" }}>Name (optional)</div>
            <input type="text" placeholder="z.B. Monatsgoal Mai" value={form.label} onChange={e => setForm(f => ({...f, label: e.target.value}))}
              style={{ background: "#000000", border: "1px solid #1e2436", borderRadius: 7, padding: "9px 12px", color: "#FFFFFF", fontSize: 13, width: "100%", outline: "none" }} />
          </div>
          <button onClick={addGoal}
            style={{ background: "#E31837", border: "none", borderRadius: 7, color: "#050810", padding: "10px", cursor: "pointer", fontSize: 13, fontWeight: 800 }}>
            Ziel speichern
          </button>
        </div>
      )}
      ...
      {goals.length === 0 && !showForm && (
        <div style={{ textAlign: "center", padding: 60, color: "#333333", fontSize: 14 }}>
      ...
      {goals.map(goal => {
        ...
        return (
          <div key={goal.id} style={{ background: "#111111", border: `1px solid ${done ? "#E3183744" : "#222222"}`, borderRadius: 14, padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: done ? "#E31837" : "#FFFFFF", marginBottom: 3 }}>
                  {done && "✓ "}{goal.label}
                </div>
                <div style={{ fontSize: 11, color: "#666666" }}>{gt?.label}</div>
              </div>
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000000", color: "#FFFFFF", fontFamily: "'Inter','Helvetica Neue',sans-serif", paddingBottom: 80 }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        tr:hover td { background: #ffffff08 !important; }
        button { font-family: 'Inter', sans-serif; }
        input, select, textarea { font-family: 'Inter', sans-serif; color: #FFFFFF; }
      `}</style>
      ...
      <div style={{ background: "#000000", borderBottom: "1px solid #1a1a1a", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 820, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ padding: "18px 0", display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#E31837", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>🏃</div>
            <span style={{ fontWeight: 800, fontSize: 18, letterSpacing: -0.5 }}>
              LAUF<span style={{ color: "#E31837" }}>LOG</span>
            </span>
          </div>
          <nav style={{ display: "flex", gap: 0 }}>
            {NAV.map(n => (
              <button key={n.key} onClick={() => { setView(n.key); setShowForm(false); setEditTarget(null); }}
                style={{ background: "transparent", border: "none", borderBottom: view === n.key ? "2px solid #E31837" : "2px solid transparent", padding: "20px 14px 18px", color: view === n.key ? "#FFFFFF" : "#666666", cursor: "pointer", fontSize: 11, fontWeight: view === n.key ? 700 : 400, letterSpacing: 0.8, textTransform: "uppercase", transition: "all .15s" }}>
                {n.label}
              </button>
            ))}
          </nav>
      ...
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "32px 24px" }}>
        {loading ? (
          <div style={{ textAlign: "center", padding: 80, color: "#333333" }}>Lade…</div>
        ) : (
          <>
            {(showForm || editTarget) && (
